...
hold on
plot(X,A_fc,'LineWidth',lw,'Color',Color(:,color2))
xlabel('x')
ylabel('y')
...
for ii=1:nsteps
    ...
    A_fc = M*A_fc;
    P = M*P*M'+Q;
    y = H*Ac + normrnd(0,r,4,1);
    K = P*H'/(H*P*H'+R);
    A_fc = A_fc + K*(y-H*A_fc);
    
    h1 = plot(X,A_fc,'LineWidth',lw,'Color',Color(:,color2));
    hold on
    h2 = plot(X,Ac,'LineWidth',lw,'Color',Color(:,color1));
    h3 = plot([-170,-80,10,100],y,'o','MarkerSize',sz,'Color',Color(:,color3));
    plot([-170,-80,10,100],y,'o','MarkerSize',sz-2,'Color',Color(:,color3))
    plot([-170,-80,10,100],y,'o','MarkerSize',sz+1,'Color',Color(:,color3))
    xlabel('x')
    ylabel('y')
