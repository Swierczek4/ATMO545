...
hold on
plot(X,A_fd,'LineWidth',lw,'Color',Color(:,color2))
xlabel('x')
ylabel('y')
...
for ii=1:nsteps
    ...
    A_fd = M*A_fd;
    P = M*P*M';
    y = H*Ad + normrnd(0,r,4,1);
    K = P*H'/(H*P*H'+R);
    A_fd = A_fd + K*(y-H*A_fd);
    
    h1 = plot(X,A_fd,'LineWidth',lw,'Color',Color(:,color2));
    hold on
    h2 = plot(X,Ad,'LineWidth',lw,'Color',Color(:,color1));
